...
function getArtistByIndex(arr, dex) {
  return `the artist at index ${arr[dex].id} is ${arr[dex].name}`;
}
...
function removeArtist(arr, dex) {

  // const filteredPeople = arr.filter((item) => item.id !== dex);
  // let num = filteredPeople.length;
  // return num;

  const arrIndex = arr.indexOf(dex);
